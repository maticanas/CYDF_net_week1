...
void data_print(u_char *data, int datalen)
{
    if(datalen==0)
    {
        printf("-----------------------------------\n");
...
int main() //int main(int argc, char *argv[])
{
   ...
   printf("hw1 2014400021 HyoJun Shin\n\n");
   ...
   */
   ...
   ih = (struct ip_header *)(packet+ETHERNET_LENGTH);

   if(set_ether(eh, ipv4) && set_ipv4(ih, tcp))
   {
       th = (struct tcp_header *)(packet+ETHERNET_LENGTH+offset);
       ether_print(eh);
       ip_print(ih);
       tcp_print(th);

       len_after_iph = ntohs(ih->ip_total_length) - (ih->ip_header_len * 4);
       len_th = th->data_offset * 4;
       len_data = len_after_iph - len_th;

       data_print((u_char *)th+len_th, len_data);
   }
   printf("\n===================================\n");
   printf("\n\n");
